...
export function DifficultyVote({ members = [], card, onUpdateDifficulty, currentUserId }) {
    const [votes, setVotes] = useState([]);
    const [revealed, setRevealed] = useState(false);

    const getMemberVote = (memberId) =>
        votes.find((v) => v.cardId === card.id && v.memberId === memberId)?.difficultyId ?? null;

    const currentUserVote = getMemberVote(currentUserId);

    const allVoted = members.length > 0 && members.every((m) => getMemberVote(m.id) !== null);
    ...
    const handleVote = (diffKey) => {
        ...
        const nowAllVoted = members.every(
            (m) => newVotes.find((v) => v.cardId === card.id && v.memberId === m.id) !== undefined
        );
        if (nowAllVoted) {
            const counts = { Low: 0, Medium: 0, High: 0 };
            newVotes
            ...
            const winnerKey = Object.entries(counts).reduce((a, b) => (b[1] > a[1] ? b : a))[0];
            setRevealed(true);
    ...
    return (
        <Box sx={{ display: "flex", flexDirection: "column", gap: 1.5 }}>
            ...
            <Collapse in={votedCount === 0}>
                ...
                >
                    Setiap anggota tim wajib melakukan voting difficulty sebelum estimasi dikunci.
                </Alert>
            </Collapse>
                ...
                {DIFFICULTY_OPTIONS.map((opt) => {
                    ...
                    return (
                        <Box
                            ...
                            sx={{
                                flex: 1,
                                border: `2px solid ${isSelected || isWinner ? opt.border : "#E5E7EB"}`,
                                borderRadius: "8px",
                                padding: "10px 6px",
                                display: "flex",
                        ...
                        >
                            ...
                            {revealed && (
                                <Box sx={{ width: "100%", mt: 0.5 }}>
                                    ...
                                    <Typography fontSize={10} color="text.secondary" textAlign="center" mt={0.25}>
                                        {tally[opt.key]} vote
                                    </Typography>
                                </Box>
                            )}
                            ...
                            >
                                <Box
                                    sx={{
                                        ...
                                        background: hasVoted && revealed ? voteOpt?.bg : hasVoted ? "#F0FDF4" : "#F9FAFB",
                                        transition: "all 0.2s",
                                    }}
                                >
                                    <Avatar
                                        sx={{
                                            ...
                                            height: 20,
                                            fontSize: 10,
                                            bgcolor: hasVoted ? (voteOpt?.color ?? "#16a34a") : "#9CA3AF",
                                        }}
                                    >
                                    ...
                                    <Typography
                                        fontSize={11}
                                        color={hasVoted ? (voteOpt?.color ?? "text.primary") : "text.secondary"}
                                    ...
                                    {hasVoted ? (
                                        revealed ? (
                                            <Typography fontSize={10} color={voteOpt?.color} fontWeight={700}>
                                                {voteOpt?.emoji}
                                            </Typography>
                                        ) : (
            ...
            {revealed && winner && (
                ...
                >
                    ...
                    <Box>
                        <Typography fontSize={12} fontWeight={700} color={getDiffOption(winner)?.color}>
